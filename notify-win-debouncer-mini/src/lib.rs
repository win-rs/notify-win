...
pub use notify_win;
pub use notify_win_types::debouncer_mini::{DebouncedEvent, DebouncedEventKind};

use notify_win::{Error, Event, RecommendedWatcher, Watcher};
...
/// # use notify_win_debouncer_mini::{DebounceEventHandler,DebounceEventResult};
///
...
/// ```rust
/// # use std::time::Duration;
/// use notify_win_debouncer_mini::Config;
/// let backend_config = notify_win::Config::default();
///
...
pub struct Config {
    timeout: Duration,
    batch_mode: bool,
    notify_config: notify_win::Config,
}
...
impl Default for Config {
    fn default() -> Self {
        Self {
            ...
            batch_mode: true,
            notify_config: notify_win::Config::default(),
        }
    }
}
...
pub fn new_debouncer_opt<F: DebounceEventHandler, T: Watcher>(
    ...
    std::thread::Builder::new()
        .name("notify-win debouncer loop".to_string())
        .spawn(move || {
